...
proc unscaledVariance*(x: seq[SomeNumber]): float {.inline.} = 
  let xMean: float = mean(x)
  var total: float = 0.0
  for v in x: total += pow2(v.float - xMean)
  return total


proc variance*(x: seq[SomeNumber], df: int = 1): float {.inline.} = 
  return unscaledVariance(x) / (x.len - df).float


proc pooledVariance*(x1, x2: seq[SomeNumber]): float {.inline.} = 
  return (unscaledVariance(x1) + unscaledVariance(x2)) / (x1.len + x2.len - 2).float


proc standardDeviation*(x: seq[SomeNumber], df: int = 1): float {.inline.} =
